...
prefix = "testresults/cholesky/test1test/"
...
for i = coverage_start : coverage_step : coverage_stop
    ...
    for t = 1:probes
        ...
        A = generatePosDefMatrix(n, i)
        cc_sparse_a = toSparse( A )
        julia_sparse_a = sparse( A )

        S, time, bytes, gctim, memallocs = @timed symbolic_cholesky(1, cc_sparse_a)
        addResults(symbolic_results, time, bytes, probes)

        _, time, bytes, gctim, memallocs = @timed cholesky_fact(cc_sparse_a, S)
        addResults(ccsparse_results, time, bytes, probes)

        _, time, bytes, gctim, memallocs = @timed cholfact(julia_sparse_a)
        addResults(julia_sparse_results, time, bytes, probes)

        _, time, bytes, gctim, memallocs = @timed chol(A)
